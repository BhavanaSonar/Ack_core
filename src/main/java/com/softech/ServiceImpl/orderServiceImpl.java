...
import com.softech.Repository.acDocumentRepo;
...
public class orderServiceImpl implements orderService {
	...
	@Autowired
	private acDocumentRepo acDocuments;
	...
	public orderHeader updateOrderHeader(orderHeader order, Long PoNumber) {
		...
		acDocuments acD= new acDocuments();
		acD.setAcknowledgementsType("AC");
		acD.setUserName(orderHeader.getUserName());
		acD.setPurchaseOrderNumber(orderHeader.getPurchaseOrderNumber());
		acD.setOrderItems(orderHeader.getOrderItems());
		
		acDocuments.save(acD);
